...
export class Bullet extends EventEmitter {
    ...
    collide(sprite) {
        ...
        const bulletPosition = this.sprite.getGlobalPosition();

        if (bulletPosition.x >= enemyBounds.left &&
            bulletPosition.x <= enemyBounds.right &&
            bulletPosition.y >= enemyBounds.top &&
            bulletPosition.y <=  enemyBounds.bottom
        ) {
            return true;
        }
        return false;
    ...
    update() {
        if (!App.pauseExecution) {
           ...
           if (position.x < 0 || position.x > App.app.canvas.width) {
              this.remove();
           }
        }
    }
