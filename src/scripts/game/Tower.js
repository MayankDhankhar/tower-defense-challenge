...
export class Tower extends Tile {
    ...
    attack(enemy) {
        ...
        if (this.active) {
            ...
            gsap.to({x: 0}, 
                {
                    duration: this.config.cooldown / 1000,
                    onComplete: () => {
                        this.active = true;
                    },
                    ease: "none"
    ...
    createArea() {
        ...
        this.area.circle(this.sprite.getGlobalPosition().x, this.sprite.getGlobalPosition().y, this.config.range);
        this.area.fill("white");
    }
