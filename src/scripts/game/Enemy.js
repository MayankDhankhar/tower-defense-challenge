...
export class Enemy extends Tile {
    ...
    constructor(config, path, enemyDelay) {
        super(config.id);
        this.config = config;
        ...
        this.createTween(enemyDelay);
    }

    createTween(enemyDelay) {
        this.tween = gsap.to({x: 0}, 
            {
                duration: enemyDelay / 1000,
                onComplete: () => {
                    this.move();
                },
                paused: App.pauseInitialWave,
                ease: "none"
            });
        // this.tween.pause();    
    }
    ...
    get isOutOfTheScreen() {
        if (this.pathIndex === this.path.length) {
            ...
            if (point.x < 0 ||point.x > App.app.canvas.width) {
                return true;
            }
        }
